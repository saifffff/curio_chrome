...
function loadPopups() {
    for (; queuedPopups.length > 0;)
        queuedPopups.shift().init()
} link.rel = "stylesheet",
    link.type = "text/css", link.href = "https://cdn.jsdelivr.net/npm/@simondmc/popup-js@1.4.2/popup.min.css",
    link.media = "all", head.appendChild(link), link.onload = function () { loadPhase += 1, 2 === loadPhase && loadPopups() },
    window.addEventListener("load", (() => { loadPhase += 1, 2 === loadPhase && loadPopups() }));
class Popup {
    constructor(t = {}) { this.params = t, 2 == loadPhase ? this.init() : queuedPopups.push(this) } init() {
        this.id = this.params.id ?? "popup", this.title = this.params.title ?? "Popup Title",
            this.content = this.params.content ?? "Popup Content", this.titleColor = this.params.titleColor ?? "#000000",
            ...
            this.heightMultiplier = this.params.heightMultiplier ?? .66,
            this.fontSizeMultiplier = this.params.fontSizeMultiplier ?? 1,
            this.borderRadius = this.params.borderRadius ?? "15px",
            this.sideMargin = this.params.sideMargin ?? "3%", this.titleMargin = this.params.titleMargin ?? "2%",
            this.lineSpacing = this.params.lineSpacing ?? "auto",
            this.showImmediately = this.params.showImmediately ?? !1, this.showOnce = this.params.showOnce ?? !1,
            this.fixedHeight = this.params.fixedHeight ?? !1, this.allowClose = this.params.allowClose ?? !0,
            this.underlineLinks = this.params.underlineLinks ?? !1, this.fadeTime = this.params.fadeTime ?? "0.3s",
            this.buttonWidth = this.params.buttonWidth ?? "fit-content", this.borderWidth = this.params.borderWidth ?? "0",
            ...
            this.hideCloseButton = this.params.hideCloseButton ?? !1, this.hideTitle = this.params.hideTitle ?? !1,
    ...
    } show() { this.popupEl.classList.remove("fade-out"), this.popupEl.classList.add("fade-in"), postShow(this.params.disableScroll ?? !0) } hide() { this.popupEl.classList.remove("fade-in"), this.popupEl.classList.add("fade-out"), localStorage && this.showOnce && localStorage.setItem("popup-" + this.id, !0), postHide(this) }
} function postShow(t) { t && disableScroll() } function postHide(t) { t.params.hideCallback && "function" == typeof t.params.hideCallback && t.params.hideCallback(), enableScroll() } function disableScroll() { const t = window.scrollY || document.documentElement.scrollTop, i = window.scrollX || document.documentElement.scrollLeft; window.onscroll = function () { window.scrollTo(i, t) } } function enableScroll() { window.onscroll = function () { } }
